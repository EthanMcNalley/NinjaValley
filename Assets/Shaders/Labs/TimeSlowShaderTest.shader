Shader "TimeSlowShader"
{
	Properties
    {
		_Strength ("Strength", Range(0,20)) = 1
		_WipeOriginPoint ("WipeOrigin", Vector) = (0.5,0.5,0,0)
		_WipeSize ("WipeSize", Range(0,3)) = 1
    	_ClearSize ("ClearSize", Range(0,3)) = 0
		_WipeThickness ("WipeThick", Range(0,1)) = 1
		_NoiseScale ("NScale", Range(0,20)) = 1
		_NoiseStrength ("NStrength", Range(0,20)) = 1
		_HighlightSize ("Hsize", Range(0,20)) = 1
		_HighlightStrength ("HighStrenght", Vector) = (0.1,0.25,0,0)
		_HighlightSpeed ("HSpeed", Range(0,20)) = 1
		_HighlightThresholds ("HighThresh", Vector) = (0.9,1,0,0)
		_EdgeColor ("EdgeColor", Color) = (0,0,0,0)
    	_ClearEdgeColor ("ClearEdgeColor", Color) = (0,0,0,0)
	...
	SubShader
	{
		...
		Pass
		{
            ...
            float4 frag (Varyings i) : SV_Target
            {
            	...
            	float insideApply = 1.0f - step(_WipeSize, distance);
				float insideClear = 1.0f - step(_ClearSize, distance);
            	
            	float active = saturate(insideApply - insideClear);

				float timer = (sin((noise * _HighlightSize + _Time.y * _HighlightSpeed) * PI) + 1.0f) * 0.5f;
				float3 recallColor = active * mask * 
					(_HighlightStrength.x + smoothstep(_HighlightThresholds.x, _HighlightThresholds.y, timer) * 
					_HighlightStrength.y) * _EdgeColor;
				float greyscaleColor = Luminance(col.rgb) * _TintColor;
				

				col.rgb = lerp(col.rgb + recallColor, greyscaleColor, _Strength * (1.0f - mask) * active);

				float applyEdge = step(_WipeSize - _WipeThickness, distance) * insideApply;
				col.rgb = lerp(col.rgb, _EdgeColor, applyEdge);
            	
				float clearBand = step(_ClearSize - _WipeThickness, distance) * (1.0f - step(_ClearSize, distance)); 
				float isClearing = step(0.0001f, _ClearSize);
				float clearEdge = clearBand * insideApply * isClearing; 

				col.rgb = lerp(col.rgb, _ClearEdgeColor, clearEdge);
